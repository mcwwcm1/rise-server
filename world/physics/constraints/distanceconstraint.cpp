...
void DistanceConstraint::ApplyConstraint(DynamicEntity* entity, Movement* movement)
{
	...
	if(delta.magnitudeSquared() > distance * distance)
	{
		...
		if(isRigid)
		{
			Double3 displacement = (delta.normalized() * (distance - delta.magnitude()));

			DynamicEntity* dynamicEntity2 = dynamic_cast<DynamicEntity*>(targetEntity);
			...
			{ 
				...
				entity->AddImpulseForceAtPosition(relativeVelocity * dot(relativeVelocity.normalized(), tugDirection * -1) / massRatio, globalAttachmentPointFrom);
				dynamicEntity2->AddImpulseForceAtPosition(relativeVelocity * dot(relativeVelocity.normalized(), tugDirection) * massRatio, globalAttachmentPointTarget);
			}
			else
			{
				movement->toPosition += displacement;
				entity->AddImpulseForceAtPosition(entity->velocity * dot(entity->velocity.normalized(), displacement.normalized()), globalAttachmentPointFrom);
			}
		}
		else
		{
			// Apply tensile force
			entity->AddForceAtPosition(delta * (distance - delta.magnitude()) * tensileForce, globalAttachmentPointFrom);
		}	
	}
}