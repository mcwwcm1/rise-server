...
void EchoToParser(std::string& arguments)
{
	...
	argumentBuffer.put(PrimaryArgument(new std::string(arguments.substr(0, arguments.find(" ")))));
	argumentBuffer.put(PrimaryArgument(new std::string(arguments.substr(arguments.find(" ") + 1, arguments.length()))));
	// Unlock buffers
