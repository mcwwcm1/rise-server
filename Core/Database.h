...
void DbConnect();
//this creates a player (or crew). If the player/crew already exists, it currently exits without creating them or throwing an error.
void CreatePlayer(const std::string& userID, int qpCount, std::string location);
//returns a user's (or crew's) qp count.
...
//alters a user's (or crew's) qp count. The passed delta is how many qpies to add/subtract from their current balance.
...
//Alters how many of a specific item a user/crew has. If the item does not exist yet, an entry will be created for it with the specified amount.
//If the item does exist the amount indicates how many of the item to add or remove from a player's (or crew's) inventory.
...
//retrieves the amount a player/crew has of an item in their inventory.
uint64_t GetInventoryItemCount(const std::string& userID, const std::string& itemLabel);
//deletes a player/crew and all associated data from the database.
